...
for spkr in [spkr_num]:
    ...
    if normalise_dut:           # correct response via DUT filter (if used!)
        norm_filter = convfilt(
            norm_filter,
            dut_norm,
            'full'
            )[int(dut_norm_sfile.nframes)/2: \
              int(dut_norm_sfile.nframes)/2 + kirN]

    # convert to length kirN - 1
    norm_filter = convfilt(
        norm_filter,
        real(fir_hb(kirN)),
        'full'
        )[kirN / 2: kirN / 2 + kirN - 1]

    # trimmed and window?
    if drcN < kirN - 1:
        norm_filter = norm_filter[(kirN - drcN) / 2: (kirN - drcN) / 2 + drcN]
        norm_filter *= kaiser(drcN, pi)
