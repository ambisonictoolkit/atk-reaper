...
# ******************************************************
# 
# Generate log sine sweep (LSS) for DUT measurement
...
file_encoding = 'pcm24'         # note: only applies to the signal
                                # not the decon filter. Decon filter
                                # is set to 'pcm32

freq0 = sr/2.**13               # log sweep start
#freq0 = sr/2.**14               # log sweep start
##freq0 = sr/2.**16               # log sweep start
...
rtime = 0.1                     # expected rt, in secs
                                # use short vals for DUT
...
if not os.path.exists(os.path.join(working_dir, str(sr), signal_dir)):
    os.makedirs(os.path.join(working_dir, str(sr), signal_dir))
