...
def classic_kernels(M, pos, k, subject_id, hrir_database_dir,
    ...
    proto_kernels = lHRIR_decoder_kernel(
        pos,
        k,
        subject_id,
        hrir_database_dir,
...
psycho_freq = 700.
band_freqs = [NaN, 60., 2000., 18000., NaN];   # filter bands
...
azimuths = arange(0, 360, 15)           # azimuths
elevations = arange(45, -60, -15)       # elevations
mesh_az, mesh_el = meshgrid(azimuths, elevations)

pos = array([]) # empty
for i in range(shape(mesh_az)[0]):
    for j in range(shape(mesh_az)[1]):
        pos = append(pos, 1)
        pos = append(pos, deg_to_rad(mesh_az[i, j]))
        pos = append(pos, deg_to_rad(mesh_el[i, j]))
        
pos = reshape(pos, (len(azimuths) * len(elevations), 3))
pos = pos[:len(pos)/2]
pos = spher_to_cart(pos)
