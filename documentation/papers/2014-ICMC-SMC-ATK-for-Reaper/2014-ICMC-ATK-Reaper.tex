...
Distance is described in meters, and to prevent bass boost from getting out of hand, the lower limit is set to 10 cm (0.1 m).
...
\emph{Proximity} and \emph{NearFieldCompensation} respectively apply an integrator or high pass filter\footnote{A 1\textsuperscript{th}-order high pass filter is the reciprocal of a 1\textsuperscript{th}-order integrator.} to the first order components of the encoded signal.
...
Balance and phase relations between the four channels of the FOA signal are critical for the spatial information.
If one or more encoded B-format channels are processed using filters, distortion, delays, or some other audio effect, the spatial information of the encoded signal will most likely be distorted or otherwise lost, often resulting in distorted or unintended spatial results. This will tend to be the case if processing effects the individual B-format channels individually, i.e., if un-synced changes in gain or phase occur. 
