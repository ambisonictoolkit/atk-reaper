...
\emph{Normalised 3D formulas (N3D)} and \emph{semi-normalised 3D formulas (SN3D)} can be used for higher order encoding and decoding  \cite{daniel:2001phd}, and e.g., the ICST ambisonic externals for Max supports up to 11th order using N3D and SN3D \cite{schacher2010:seven}.
...
With respect to azimuths, it would be preferable to be able to use horizontal sliders with positive values to the left and negative values to the right, ensuring consistency with the SuperCollider implementation of ATK.
This way moving the slider to the right would also move the source direction to the right for angles in the $(-90^{\circ}, 90^{\circ})$ range.
At the same time automation curve values would increasing with clockwise rotations, similar to what happens when moving a regular stereo pan pot to the right.
Azimuths are implemented this way in most ambisonic plugins, with the exception of WigWare, where a clockwise change of azimuth angle results in a decreasing automation curve value.
Unfortunately the Reaper JSFX API does not currently support horizontal sliders with increasing values to the left, and for this reason azimuths for the time being are described using a navigational coordination system.
If Reaper in the future is updated to support sliders with increasing values to the left, this design decision will be revisited.
