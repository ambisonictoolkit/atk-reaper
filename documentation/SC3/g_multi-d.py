...
# dominance only, includes 'hard wire' for w = 90deg
...
trans_list = [
    (mu.dominate_x, 'Dominance', 'dominance_fig')
]
...
# t_angle_num = 3                 # number of transform angles to plot (=num plots)
...
# t_angle is actually gain, rather than angle
# t_angle_min = 0.                # min angle in radians
# t_angle_max = mu.halfPi         # max "     "  "
t_angle_min = 0.                # min gain in dB
t_angle_max = 24.               # max "     "  "
...
#     fig_title = '$\\lambda = ' + str(t_angle)[:-2] + '\/dB$' # include dB
    fig_title = '$\/g = ' + str(t_angle)[:-2] + '\/dB$' # include dB
